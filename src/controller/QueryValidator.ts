    ...
    */
        ...
        if (query && typeof query === "object") {
            if (this.equals(Object.keys(query), ["WHERE", "OPTIONS"]) ||
                ...
                if (query["WHERE"].hasOwnProperty) {
                    ...
                    } else { throw new InsightError("Invalid WHERE structure"); }
                } else {
    ...
    private checkWHERE(filter: any, parent: string, datasetToQuery: string): boolean {
        if (parent === "OR" || parent === "AND") {
            ...
            try { logicValid = this.checkLogicComp(filter, parent, datasetToQuery); } catch (err) { throw err; }
            return logicValid;
        }
        const that = this;
        let listOfKeys: string[];
        try { listOfKeys = Object.keys(filter);
        } catch (err) {
        ...
        let valid: boolean;
        if (parent === "NOT") {
            try { valid = this.checkWHERE(filter[listOfKeys[0]], listOfKeys[0], datasetToQuery);
            } catch (err) { throw err; }
            return valid;
        } else if (parent === "IS") {
            try {
    ...
    private checkLogicComp(listOfFilters: any, parent: string, datasetToQuery: string): boolean {
        ...
        for (const filter of listOfFilters) {
            let listOfKeys: string[];
            try { listOfKeys = Object.keys(filter);
            } catch (err) { throw new InsightError(parent + " expects filters, got " + filter + " instead"); }
            if (listOfKeys.length !== 1) {
            ...
            if (!validFilter) { throw new InsightError("Invalid " + listOfKeys[0]); }
        }
        return true;
    }
    private checkFilterCOMP(filter: any, parent: string, datasetToQuery: string, expected: string): boolean {
        ...
        let rt: boolean;
        try { rt = this.checkKeys(listOfKeys[0], expected, filter);
        } catch (err) { throw err; }
        return rt;
    }
    ...
    private checkKeys(key: string, expected: string, filter: any): boolean {
        ...
        if (this.coursevalidator.hasOwnProperty(key)) {
            if (this.coursevalidator[key] === valueType && valueType === expected) { return expected === valueType;
            } else { throw new InsightError(filter + " expects " + expected +
    ...
    private checkOPTIONS(option: any, parent: string): boolean {
        ...
        if (parent === "OPTIONS") {
            if (listOfKeys.includes("COLUMNS")) {
                ...
                try { validColumns = this.checkCOLUMNS(option["COLUMNS"]);
                } catch (err) { throw err; }
                if (validColumns) {
                    if (listOfKeys.includes("ORDER")) {
                        ...
                        try { validOrder = this.checkORDER(option["ORDER"], option["COLUMNS"]);
                        } catch (err) { throw err; }
                        return validOrder;
                    } else { return true; }
                }
    ...
    private checkORDER(key: string, columns: string[]): boolean {
        ...
        for (let column of Object.keys(this.coursevalidator)) { if (key === column) { validColumn = true; } }
